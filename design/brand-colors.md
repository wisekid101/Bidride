# Brand Colors

> **Status: PLACEHOLDER — Awaiting design direction from founder**

## Direction Questions
- What feeling should BidiRide evoke? (Trust? Speed? Value? Empowerment?)
- Who is the primary visual audience — riders or drivers?
- Competitor reference: Uber (black/white), Lyft (magenta/white) — differentiate or follow?

## Placeholder Palette (To Be Replaced)
| Role | Color | Notes |
|------|-------|-------|
| Primary | TBD | Main brand color |
| Secondary | TBD | Accent / CTA |
| Background | TBD | App background |
| Success | TBD | Ride confirmed, payment success |
| Warning | TBD | Bid pending, surge pricing |
| Error | TBD | Cancellation, failed payment |

---
*Last updated: 2026-06-24*
