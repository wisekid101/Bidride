...
describe('safety-service observability adoption', () => {
  it('the shared health controller reports the service identity', async () => {
    const mod = await Test.createTestingModule({
      controllers: [ObservabilityHealthController],
      providers: [
        { provide: OBSERVABILITY_OPTIONS, useValue: { serviceName: 'safety-service', version: '1.0.0' } },
    ...
    expect(live).toMatchObject({ status: 'healthy', service: 'safety-service' });
  ...
  it('readiness reports unhealthy when a required dependency fails', async () => {
    ...
    const mod = await Test.createTestingModule({
      controllers: [ObservabilityHealthController],
      providers: [
        { provide: OBSERVABILITY_OPTIONS, useValue: { serviceName: 'safety-service' } },
        {
  ...
  it('telemetry failure does not propagate to the caller', () => {
    ...
    try {
      expect(() => {
        ...
        registry.counter('bidride_safety_adoption_probe_total', 'test').inc({ tripId: 'x' });
      }).not.toThrow();
    } finally {
      capture.stop();
    }
